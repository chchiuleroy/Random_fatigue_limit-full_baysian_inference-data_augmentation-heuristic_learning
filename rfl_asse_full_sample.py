...
def gl_moments(mu_d, sd_d, S_j, norm_tol=0.01):
    ...
    2026-08-19 修正（小o code review 抓到）：norm 原本直接用 GL 加權和，
    σ_Δ 很窄時固定節點會漏掉尖峰，算出來的值可能大於1（機率不該超過1）。
    Δ~LogNormal(μ_Δ,σ_Δ) 時 P(Δ<S_j) 其實有解析解
    = Φ((ln S_j - μ_Δ)/σ_Δ)（標準常態 CDF），現在直接用這個當正規化分母，
    不再依賴 GL 積分本身的加總；GL 積分只用來算分子（E1/E2 的加權和，這兩個
    量沒有封閉解，仍要靠數值積分，128-point 已驗證在目前後驗附近收斂）。
    若 GL 加總得到的 norm 與解析解差距超過 norm_tol，代表這組 (μ_Δ,σ_Δ)
    落在固定節點會漏峰的區域，直接 raise，不要沉默地繼續算下去。
